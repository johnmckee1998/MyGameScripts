...
public class TestDestruction : MonoBehaviour
{
    ...
    {
        Debug.Log("Col hit " + collision.gameObject.name + " Mag: " + collision.impulse.magnitude);
    ...
    private void OnTriggerEnter(Collider other) //idea: raycast from bullet to object to get collision point more accuratley
    {
        if (other.GetComponent<BulletScript>()) //if its a bullet
        {
            Debug.Log("Bullet Hit " + other.GetComponent<Rigidbody>().velocity.magnitude);
            Vector3 pt = transform.InverseTransformPoint(other.transform.position);
            Vector3 nrm = transform.InverseTransformDirection(other.transform.forward*-1);
            float imp = other.GetComponent<Rigidbody>().velocity.magnitude/100;
            if (imp < minImpulse)
                return;
