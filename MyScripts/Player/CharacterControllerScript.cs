...
public class CharacterControllerScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (GlobalStats.instance != null)
            if (GlobalStats.instance.isPaused)
                return; //dont do anything while paused


        if (healthBar == null)
        {
            //get the healthbar once it is spawned
            if (CanvasScript.instance != null)
            {
                healthBar = CanvasScript.instance.healthBar;
                healthBarScale = healthBar.transform.localScale;
                healthBar.fillAmount = (health / startHealth);
            }
        }
        ...
        */
        ...
        if (healthBar != null)
        {
            if (health != prevHealth) //effects when taking damage
            {
